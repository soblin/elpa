...
(defun magit-maybe-define-global-key-bindings nil (when magit-define-global-key-bindings (let ((map (current-global-map))) (unless (lookup-key map (kbd "C-x g")) (define-key map (kbd "C-x g") 'magit-status)) (unless (lookup-key map (kbd "C-x M-g")) (define-key map (kbd "C-x M-g") 'magit-dispatch)) (unless (lookup-key map (kbd "C-c M-g")) (define-key map (kbd "C-c M-g") 'magit-file-dispatch)))))
